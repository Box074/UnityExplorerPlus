...
class tk2dClipDumpWidget : Texture2DWidget
{
    ...
    {
        progressLabel.text = $"{GetLengthString(0)} / {GetLengthString(clip.frames.Length / clip.fps)}";
    }
    ...
    private IEnumerator PlayClipCoroutine()
    {
        ...
        do
        {
            ...
            foreach (var v in clip.frames)
            {
                ...
                {
                    progressLabel.text = $"{GetLengthString(Time.unscaledTime - st)} / {GetLengthString(clip.frames.Length / clip.fps)}";
                    yield return null;
                }
            }
